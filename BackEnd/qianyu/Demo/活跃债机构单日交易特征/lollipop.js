...
d3.csv("instn_trader_trades_data.csv", function(d) {
    return {
        ...
        trade_amount: +d.trade_amount,  // 转换为数字
        trade_price: +d.trade_price     // 转换为数字
    };
}).then(function(data) {

    const minTradeTime = d3.min(data, d => d.trade_time);
    const maxTradeTime = d3.max(data, d => d.trade_time);

    const maxTradeAmount = d3.max(data, d => d.trade_amount);
    const minTradeAmount = d3.min(data, d => d.trade_amount);

    // console.log('data:', data);
    // console.log('minTradeTime:', minTradeTime, 'maxTradeTime:', maxTradeTime);
    ...
    const tradingStats = data.reduce((acc, d) => {
        ...
        acc[key].totalAmount += +d.trade_amount || 0;
    ...
    // const minTradePrice = d3.min(data, d => d.trade_price);
    // const maxTradePrice = d3.max(data, d => d.trade_price);
    ...
    const r = d3.scaleLog()
                .range([ 2, 8 ])
                .domain([minTradeAmount, maxTradeAmount]);
